...
local function main()
	...
	Properties.ExpandableTypes = {
		...
		["CFrame"] = true, ["Rect"] = true, ["PhysicalProperties"] = true,
		["Ray"] = true, ["NumberRange"] = true, ["Faces"] = true, ["Axes"] = true,
	}

	Properties.ExpandableProps = { ["Sound.SoundId"] = true }

	Properties.CollapsedCategories = { ["Surface Inputs"] = true, ["Surface"] = true }

	Properties.ConflictSubProps = {
		["Vector2"] = {"X","Y"}, ["Vector3"] = {"X","Y","Z"}, ["UDim"] = {"Scale","Offset"},
		...
		["NumberRange"] = {"Min","Max"}, ["Faces"] = {"Back","Bottom","Front","Left","Right","Top"},
		["Axes"] = {"X","Y","Z"}
	}

	Properties.ConflictIgnore = { ["BasePart"] = { ["ResizableFaces"] = true } }
	...
	Properties.TypeNameConvert = { ["number"]="double", ["boolean"]="bool" }

	Properties.ToNumberTypes = { ["int"]=true,["int64"]=true,["float"]=true,["double"]=true }
	...
	Properties.ValueToString = function(prop,val)
		local typeData = prop.ValueType
		local typeName = typeData.Name
		if typeName == "Color3" then return Lib.ColorToBytes(val)
	...
	Properties.ComputeConflicts = function(p)
		...
		if p then
			...
			for i = 1,#subProps do autoUpdateObjs[gName.."."..subProps[i]] = nil end
		else
					...
					for c = 1,#classList do
						...
						if not firstSet then
							if isAttribute then
								...
								if firstVal ~= nil then firstObj = obj firstSet = true end
							else firstVal = obj[propName] firstObj = obj firstSet = true end
								...
								if subPropCount > 0 then
									for sPropInd = 1,subPropCount do
										...
										if not conflictMap[mapInd] and firstValSub ~= propValSub then
											conflictMap[mapInd] = true conflictsFound = conflictsFound + 1
										end
									end
								end
	...
	Properties.ShowExplorerProps = function()
				...
				if not foundClasses[APIClassName] then
					...
					for i = 1,#apiProps do
						...
						if (not tags.Deprecated or showDeprecated) and (not tags.Hidden or showHidden) then
							props[propCount] = prop propCount = propCount + 1
						end
					end
			...
			if showingAttrs and attrCount < maxAttrs then
				...
				for name,val in pairs(attrs) do
					...
					if not foundAttrs[name] then
						...
						props[propCount] = attrProp propCount = propCount + 1
		...
		table.sort(props,function(a,b)
			if a.Category ~= b.Category then
				...
				if aOrder ~= bOrder then return aOrder < bOrder
				else return lower(a.Name) < lower(b.Name) end
			end
		end)
	...
	Properties.NewPropEntry = function(index)
		...
		return {
			...
			GuiElems = {
				...
				Expand = nameFrame.Expand, ColorButton = valueFrame.ColorButton,
				ColorPreview = valueFrame.ColorButton.ColorPreview,
				Gradient = valueFrame.ColorButton.ColorPreview.UIGradient,
				EnumArrow = valueFrame.EnumArrow, Checkbox = valueFrame.Checkbox,
				RightButton = valueFrame.RightButton, RightButtonIcon = iconFrame,
	...
	Properties.DisplayAddAttributeWindow = function(editAttr)
		...
		if not win then
			...
			saveButton.OnClick:Connect(function()
				...
				if #name > 100 then errorLabel.Text = "Error: Name over 100 chars" return
				elseif name:sub(1,3) == "RBX" then errorLabel.Text = "Error: Name begins with 'RBX'" return end
	...
	Properties.DisplayEnumDropdown = function(entryIndex)
		...
		local sorted = {}
		for name,enum in next,enum:GetEnumItems() do sorted[#sorted+1] = enum end
		...
		for i = 1,#sorted do context:Add({Name=sorted[i].Name,OnClick=onClick}) end
	...
	Properties.DisplayBrickColorEditor = function(prop,entryIndex,col)
		...
		if not editor then
			...
			editor.OnMoreColors:Connect(function()
				...
				for i,v in pairs(API.Classes.BasePart.Properties) do
					if v.Name == "Color" then colProp = v break end
				end
		...
		else editor:SetMoreColorsVisible(false) end
	...
	Properties.DisplayColorEditor = function(prop,col)
		...
		editor.CurrentProp = prop
		if col then editor:SetColor(col)
		else
	...
	Properties.DisplayNumberSequenceEditor = function(prop,seq)
		...
		editor.CurrentProp = prop
		if seq then editor:SetSequence(seq)
		else
			local firstVal = Properties.GetFirstPropVal(prop)
	...
	Properties.GetPropVal = function(prop,obj)
		...
		if prop.IsAttribute then
			...
			elseif prop.ValueType.Name ~= currentType then return nil end
		else propVal = obj[prop.Name] end
	...
	Properties.SelectObject = function(obj)
		if inputProp and inputProp.ValueType.Category == "Class" then
			local prop = inputProp
			inputProp = nil
			if isa(obj,prop.ValueType.Name) then Properties.SetProp(prop,obj)
			else Properties.Refresh() end
			return true
		end
		return false
	end
	...
	Properties.DisplayProp = function(prop,entryIndex)
		...
		if (typeName == "Color3" or typeName == "BrickColor" or typeName == "ColorSequence") then
			colorButton.Visible = true enumArrow.Visible = false
			if propVal then
			...
			else gradient.Color = ColorSequence.new(Color3.new(1,1,1)) end
		...
		elseif typeData.Category == "Enum" then
			colorButton.Visible = false enumArrow.Visible = not prop.Tags.ReadOnly endOffset = 22
		elseif (gName == inputFullName and typeData.Category == "Class") or typeName == "NumberSequence" then
			colorButton.Visible = false enumArrow.Visible = false endOffset = 26
		else colorButton.Visible = false enumArrow.Visible = false end
		...
		if inputFullName == gName and typeData.Category == "Class" then
			...
			rightButton.Text = "" rightButton.Visible = true
		elseif typeName == "NumberSequence" or typeName == "ColorSequence" then
			guiElems.RightButtonIcon.Visible = false rightButton.Text = "..." rightButton.Visible = true
		else rightButton.Visible = false end

		if typeName == "bool" or typeName == "PhysicalProperties" then
			valueBox.Visible = false checkbox.Visible = true soundPreview.Visible = false
			...
			else checkboxes[entryIndex]:SetState(propVal) end
		elseif typeName == "SoundPlayer" then
			valueBox.Visible = false checkbox.Visible = false soundPreview.Visible = true
			...
			valueBox.Visible = true checkbox.Visible = false soundPreview.Visible = false
			if propVal ~= nil then
				if typeName == "Color3" then valueBox.Text = "["..Lib.ColorToBytes(propVal).."]"
				elseif typeData.Category == "Enum" then valueBox.Text = propVal.Name
				...
				else valueBox.Text = Properties.ValueToString(prop,propVal) end
			else valueBox.Text = "" end
			...
			if prop then
				...
				if prop.SpecialRow then
					if prop.SpecialRow == "AddAttribute" then
						nameFrame.Visible = false valueFrame.Visible = false guiElems.RowButton.Visible = true
					end
				else
					nameFrame.Visible = true guiElems.RowButton.Visible = false
		...
		for i = 1,#sList do
			...
			if isa(obj,propClass) then
				pcall(function()
					local setVal = val
					local root
					if prop.IsAttribute then root = getAttribute(obj,attributeName)
					...
					if rootTypeName then
						...
						elseif rootTypeName == "Faces" then
							...
							for _,face in pairs(faceList) do
								local val
								if subName=="."..face then val = setVal else val = root[face] end
						...
						elseif rootTypeName == "Axes" then
							...
							for _,axe in pairs(axesList) do
								local val
								if subName=="."..axe then val = setVal else val = root[axe] end
	...
	Properties.Init = function()
		...
		window.GuiElems.Main:GetPropertyChangedSignal("AbsoluteSize"):Connect(function()
			if Properties.Window:IsContentVisible() then Properties.UpdateView() Properties.Refresh() end
		end)
		window.OnActivate:Connect(function() Properties.UpdateView() Properties.Update() Properties.Refresh() end)
		window.OnRestore:Connect(function() Properties.UpdateView() Properties.Update() Properties.Refresh() end)
